...
The \textit{Triad} method also aims to strengthen the relationship between body and movement. As discussed in TODO, Evolution Gym's co-design GA mutated these two mechanisms in tandem and produced higher-performing robots in most tasks being tested. \citepmain{evolutionGym}\\
...
Evolution Gym, \citepmain{evolutionGym} unlike the previous works covered in this section, modelled their robots with a primitive design rather than build them. It shared a common goal with Lizardbot: optimise the movement and body together using a co-design algorithm. \\
...
\section{Results}
\label{sec:Results}
%TODO explain about capping population to 50 - issues with laptop
...
As figure TODO shows, the serpentine motion showed $\sim$5x the success of the random oscillation. Thus, serpentine motion became the default motion used in future experiments (supported by the replicated high performances). 
...
The results, shown in figure TODO, suggest that forcing a uniform body hindered the robots. There was next-to-no progress made by the uniform version of the robots after 300s. \\
...
As explained in TODO, there is the option to restrict the GA to only movement or physical genes. There 18 permutations available between the mutation constraints and the recombination methods. Providing data for each would have been time-consuming (and at this point my laptop was making unhappy noises). The decision was made to instead find the most effective constraint (using the \textit{Any} recombination) and use this as the basis for testing each recombination method.\\
As predicted, allowing the manipulation of all genes produced consistently high results. Interestinly, one iteration limited to physical genes vastly exceeded the performance of all others. However, this result was not shared by the two repeats. 
...
\begin{wrapfigure}[15]{l}{0.5\textwidth}
    ...
    \vspace*{-7mm}
    \caption{The results of each recombination method.}
\end{wrapfigure}

%TODO what impact do the various methods have on performance?
localised breeding groups within the population. To balance the  and could benefit from the implementation of incest prevention. \citepmain{gaIntro}
%TODO what happens when the mutation & recombination are limited to either physical or movement, or both
%Removed outlier random - 531.8821
...
%TODO what does a population of uniform, serpentine, tailed, gaited robots evolve to?

\subsection{Impact of Terrain}
\label{sec:Terrain Res}
...
%TODO attach images of high performing robots here
